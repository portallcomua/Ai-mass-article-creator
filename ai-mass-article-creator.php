...
final class AI_Mass_Article_Creator_3 {
    ...
    private function js() {
        return 'jQuery(function($){
            $("#amac-generate-form").on("submit",function(e){
                ...
                fd.append("action","amac_generate");
                fd.append("nonce",AMAC3.nonce);
                $.ajax({
                    url:AMAC3.ajax_url,
                    type:"POST",
                    data:fd,
                    ...
                    success:function(r){
                        if(r.success){
                            ...
                            if(r.data.results&&r.data.results.length){
                                ...
                                $.each(r.data.results,function(i,p){
                                    h+="<tr><td>"+(i+1)+"</td><td>"+p.title+(p.error?"<br><span class=\\"amac-error\\">"+p.error+"</span>":"")+"</td><td>"+p.score+"/100</td><td>"+(p.image?"✅":"❌")+"</td><td>"+(p.edit_url&&p.edit_url!=="#"?"<a target=\\"_blank\\" href=\\""+p.edit_url+"\\">Редагувати</a>":"—")+"</td></tr>";
                                });
                                h+="</table>";
                                $("#amac-log").append(h);
                            }
                    ...
                    error:function(x){
                        $("#amac-progress span").css("width","100%").text("AJAX error");
                        $("#amac-log").append("<div class=\\"amac-error\\">❌ AJAX error: "+x.status+" "+x.statusText+"</div>");
                        $("#amac-generate-btn").prop("disabled",false);
                    }
                });
            });
            $(".amac-test-provider").on("click",function(e){
                e.preventDefault();
                var p=$(this).data("provider"),el=$(this);
                el.text("...");
                $.post(AMAC3.ajax_url,{action:"amac_test_provider",provider:p,nonce:AMAC3.nonce},function(r){
                    el.text(r.success?"✅ OK":"❌ Error");
    ...
    public function page_dashboard() {
        ...
        <div class="amac-grid">
            <div class="amac-card">
                <h3>Статус</h3>
                <p><strong>Згенеровано:</strong> <?php echo intval($generated); ?></p>
                <p><strong>Залишилось:</strong> <?php echo esc_html($remaining); ?></p>
                <p><strong>AI:</strong> <?php echo esc_html(strtoupper($s['ai_provider'])); ?></p>
                <p><strong>Ліцензія:</strong> <?php echo $s['license_valid'] ? '<span class="amac-ok">активна</span>' : '<span class="amac-bad">free/test</span>'; ?></p>
            </div>

            <div class="amac-card">
                <h3>Швидкий старт</h3>
                <ol>
                    <li>Введи Groq або OpenAI key у Settings.</li>
                    <li>Для якісних фото додай Pexels/Pixabay/Unsplash.</li>
                    <li>Увімкни Developer/Test Mode для тестування без ліміту.</li>
                    <li>Згенеруй 1 статтю в чернетку.</li>
                </ol>
                <p><a class="button" href="<?php echo esc_url(admin_url('admin.php?page=amac-settings')); ?>">Налаштування</a></p>
            </div>
        </div>

        <div class="amac-card" style="margin-top:18px">
            <h2>Генерація статей</h2>

            <form id="amac-generate-form">
                <div class="amac-field">
                    <label>Тема / ключове слово</label>
                    <input name="topic" required placeholder="Напр. жіноча білизна, спорт, авто, книги, промисловість, WordPress SEO">
                </div>
                ...
                <div class="amac-two">
                    <div class="amac-field">
                        ...
                        <select name="count">
                            <?php for ($i = 1; $i <= 20; $i++) echo '<option value="'.esc_attr($i).'" '.selected($i,1,false).'>'.esc_html($i).'</option>'; ?>
                        </select>
                    </div>
                    ...
                    <div class="amac-field">
                        <label>Довжина</label>
                        <select name="words">
                            <option value="600">600 слів</option>
                            <option value="900" selected>900 слів</option>
                            <option value="1300">1300 слів</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="amac-two">
                    <div class="amac-field">
                        ...
                        <select name="category">
                            <option value="0">AI створить/обере</option>
                            <?php foreach ($cats as $c) echo '<option value="'.esc_attr($c->term_id).'">'.esc_html($c->name).'</option>'; ?>
                        </select>
                    </div>
                ...
                <div class="amac-checkboxes">
                    <strong>Функції</strong>
                    <label><input type="checkbox" name="featured" checked> Головне фото</label>
                    <label><input type="checkbox" name="inline" checked> 2–3 фото в статті</label>
                    <label><input type="checkbox" name="seo" checked> SEO Rank Math / Yoast</label>
                    <label><input type="checkbox" name="faq" checked> FAQ + JSON-LD Schema</label>
                    <label><input type="checkbox" name="links" checked> Внутрішня перелінковка</label>
                    <label><input type="checkbox" name="comments"> Реалістичні коментарі</label>
                </div>

                <div id="amac-progress" class="amac-progress"><span>0%</span></div>
                <div id="amac-log" class="amac-log"></div>
                <p><button id="amac-generate-btn" class="button button-primary button-large">🚀 Генерувати</button></p>
            </form>
        </div></div><?php
    }
    ...
    public function page_settings() {
        ...
        <form method="post" class="amac-grid">
            <?php wp_nonce_field('amac_save_settings'); ?>
            ...
            <div class="amac-card">
                ...
                <div class="amac-field">
                    ...
                    <select name="amac_ai_provider">
                        <option value="groq" <?php selected($s['ai_provider'], 'groq'); ?>>Groq</option>
                ...
                <div class="amac-field">
                    ...
                    <input type="password" name="amac_api_key" value="<?php echo esc_attr($s['groq_key']); ?>">
                    <p><a href="#" class="button amac-test-provider" data-provider="groq">Перевірити Groq</a></p>
                </div>

                <div class="amac-field">
                    <label>OpenAI API Key</label>
                    <input type="password" name="amac_openai_key" value="<?php echo esc_attr($s['openai_key']); ?>">
                    <p><a href="#" class="button amac-test-provider" data-provider="openai">Перевірити OpenAI</a></p>
                </div>
            </div>
            ...
            <div class="amac-card">
                <h3>Фото</h3>
                <div class="amac-help">
                    Порядок: Pexels → Pixabay → Unsplash → Openverse.
                    AI сам створює англомовний фотозапит і negative keywords.
                    Якщо фото не релевантне — краще не вставити фото, ніж вставити комп’ютер або корову.
                </div>

                <div class="amac-field">
                    <label>Pexels API Key</label>
                    <input type="password" name="amac_pexels_key" value="<?php echo esc_attr($s['pexels_key']); ?>">
                    <span class="amac-muted">pexels.com/api</span>
                </div>

                <div class="amac-field">
                    <label>Pixabay API Key</label>
                    <input type="password" name="amac_pixabay_key" value="<?php echo esc_attr($s['pixabay_key']); ?>">
                    <span class="amac-muted">pixabay.com/api/docs</span>
                </div>

                <div class="amac-field">
                    <label>Unsplash Access Key</label>
                    <input type="password" name="amac_unsplash_key" value="<?php echo esc_attr($s['unsplash_key']); ?>">
                    <span class="amac-muted">unsplash.com/developers</span>
                </div>
            </div>
            ...
            <div class="amac-card">
                ...
                <div class="amac-field">
                    ...
                    <select name="amac_default_category">
                        <option value="0">AI/не вибрано</option>
                        <?php foreach ($cats as $c) echo '<option value="'.esc_attr($c->term_id).'" '.selected($s['default_category'],$c->term_id,false).'>'.esc_html($c->name).'</option>'; ?>
                    </select>
                </div>

                <div class="amac-field">
                    <label>Мова інтерфейсу/контенту</label>
                    <select name="amac_language">
                        <option value="uk" <?php selected($s['language'], 'uk'); ?>>Українська</option>
                        <option value="en" <?php selected($s['language'], 'en'); ?>>English</option>
                    </select>
                </div>

                <div class="amac-field">
                    <label>Посилання на товар WooCommerce</label>
                    <input name="amac_product_url" value="<?php echo esc_attr($s['product_url']); ?>">
                </div>

                <label>
                    <input type="checkbox" name="amac_dev_mode" value="1" <?php checked($s['dev_mode']); ?>>
                    Developer/Test Mode без ліміту
                </label>

                <p><button class="button button-primary">Зберегти</button></p>
            </div>
        </form></div><?php
    }
    ...
    public function page_license() {
        ...
        <div class="amac-grid">
            <div class="amac-card">
                <h3>Ліцензія</h3>
                <p>Продаж через WooCommerce/PayPal. Повна перевірка через UAServer Hub буде додана наступним релізом без зміни інтерфейсу.</p>
                <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                    <input type="hidden" name="action" value="amac_license_verify">
                    <?php wp_nonce_field('amac_license'); ?>
                    <div class="amac-field">
                        <label>License Key</label>
                        <input name="license_key" value="<?php echo esc_attr($s['license_key']); ?>">
                    </div>
                    <p>
                        <button class="button button-primary">Активувати</button>
                        <a class="button" target="_blank" href="<?php echo esc_url($s['product_url']); ?>">Купити</a>
                    </p>
                </form>
            </div>
            ...
            <div class="amac-card">
                <h3>Умови</h3>
                <ul>
                    <li>Free: <?php echo intval($this->free_limit); ?> статей.</li>
                    <li>Pro: до 3 доменів.</li>
                    <li>localhost не рахується.</li>
                </ul>
            </div>
        </div></div><?php
    }
    ...
    public function ajax_test_provider() {
        ...
        if (!current_user_can('manage_options')) {
            wp_send_json_error(array('message' => 'No access'));
        }

        $p = sanitize_text_field($_POST['provider'] ?? 'groq');
        $r = $this->ai_call('Reply OK only.', 10, $p, 1);
    ...
    public function ajax_generate() {
        ...
        if (!$s['groq_key'] && !$s['openai_key']) {
            wp_send_json_error(array('message' => 'Введіть Groq або OpenAI API Key у Settings.'));
        }

        $generated = (int) get_option('amac_total_generated', 0);

        if (!$s['license_valid'] && !$s['dev_mode'] && $generated >= $this->free_limit) {
            wp_send_json_error(array('message' => 'Free-ліміт 20 статей вичерпано. Активуйте ліцензію або увімкніть Developer Mode для тесту.'));
        }

        $topic = sanitize_text_field(wp_unslash($_POST['topic'] ?? ''));

        if (!$topic) {
            wp_send_json_error(array('message' => 'Тема порожня'));
        }

        $count = min(20, max(1, (int)($_POST['count'] ?? 1)));

        if (!$s['license_valid'] && !$s['dev_mode']) {
            $count = min($count, max(0, $this->free_limit - $generated));
        }
        ...
        $titles = $count === 1 ? array($this->make_single_title($topic)) : $this->generate_titles($topic, $count);
        ...
        foreach ($titles as $idx => $title) {
            ...
            $post_id = wp_insert_post(array(
                ...
                'meta_input' => array(
                    '_amac_generated' => 1,
                    '_amac_quality_score' => (int)$data['score']
                )
            ));
            ...
            if (!empty($_POST['seo'])) {
                $this->apply_seo($post_id, $data, $topic);
            }

            if (!empty($_POST['faq'])) {
                $this->apply_faq_schema($post_id, $data['faq']);
            }
            ...
            if (!empty($_POST['featured'])) {
                $image = (bool) $this->add_featured_image($post_id, $image_plan, $title);
            }

            if (!empty($_POST['inline'])) {
                $this->add_inline_images($post_id, $image_plan, $title);
            }

            if (!empty($_POST['comments'])) {
                $this->add_comments($post_id, $title);
            }
        ...
        if (!empty($_POST['links']) && count($cluster_posts) > 1) {
            $this->link_cluster($cluster_posts);
        }
        ...
        if ($created_count === 0) {
            ...
            wp_send_json_error(array(
                'message' => 'Статті не створені. Причина: ' . $msg,
                'results' => $results
            ));
        }

        wp_send_json_success(array(
            'message' => 'Створено ' . $created_count . ' статей.',
            'results' => $results
        ));
    }

    private function make_single_title($topic) {
        $topic = trim($topic);
        if ($topic === '') return 'Нова стаття';
        return mb_strtoupper(mb_substr($topic, 0, 1, 'UTF-8'), 'UTF-8') . mb_substr($topic, 1, null, 'UTF-8');
    }

    private function generate_titles($topic, $count) {
        $p = "Create {$count} Ukrainian SEO article titles for topic: {$topic}. One per line. No numbering.";
        $r = $this->ai_call($p, 350);
        ...
        while (count($lines) < $count) {
            $lines[] = $topic . ' — практичні поради #' . (count($lines)+1);
        }
    ...
    private function generate_article_package($topic, $title, $words) {
        $prompt = "Write Ukrainian SEO article as JSON. Topic: {$topic}. Title: {$title}. Length {$words} words. JSON keys: html, meta_title, meta_description, keywords(array), tags(array), category, faq(array question/answer), score(number), image_search(string), image_negative(array). HTML tags allowed: h2,h3,p,ul,ol,li,strong,em,table,blockquote. image_search must be English stock photo query with concrete visible objects. No markdown.";

        $max = $words >= 1300 ? 3000 : ($words >= 900 ? 2400 : 1700);

        $r = $this->ai_call($prompt, $max);
        ...
        if (!$json) {
            return $this->generate_article_html_fallback($topic, $title, $words);
        }
    ...
    private function generate_article_html_fallback($topic, $title, $words) {
        $prompt = "Write natural Ukrainian SEO article. Topic: {$topic}. Title: {$title}. Length {$words} words. Return HTML only. Use h2,h3,p,ul,ol,li,strong,em,table,blockquote. No markdown, no CSS, no scripts, no images.";

        $max = $words >= 1300 ? 2600 : ($words >= 900 ? 2100 : 1500);
        ...
        if (!$html) {
            return new WP_Error('empty', 'AI повернув порожню статтю навіть у fallback HTML режимі.');
        }
    ...
    private function normalize_article_data($json, $topic, $title) {
        ...
        if (!$json['html']) {
            return new WP_Error('empty', 'Порожня стаття після очищення HTML.');
        }
        ...
        if (!isset($json['faq']) || !is_array($json['faq'])) {
            $json['faq'] = array();
        }
    ...
    private function sanitize_article_html($html) {
        ...
        $allowed = array(
            'h2'=>array(),'h3'=>array(),'p'=>array(),'ul'=>array(),'ol'=>array(),'li'=>array(),
            'strong'=>array(),'em'=>array(),'table'=>array(),'thead'=>array(),'tbody'=>array(),
            'tr'=>array(),'th'=>array(),'td'=>array(),'blockquote'=>array(),'br'=>array()
        );
    ...
    private function ai_call($prompt, $max = 1500, $provider = null, $retries = 2) {
        $s = $this->settings();
        ...
        while ($attempt <= $retries) {
            ...
            if ($provider === 'openai' && $s['openai_key']) {
                ...
                if (!$s['groq_key']) {
                    return new WP_Error('api', 'Groq key missing');
                }
            ...
            if ($code == 429 && $attempt <= $retries) {
                $wait = $this->extract_retry_wait($raw);
                sleep($wait);
                continue;
            }

            if ($code < 200 || $code >= 300) {
                return new WP_Error('api', 'AI API HTTP ' . $code . ': ' . mb_substr($raw, 0, 350));
            }
        ...
        return new WP_Error('api', 'AI API недоступний після повторних спроб.');
    }

    private function extract_retry_wait($raw) {
        $wait = 35;

        if (preg_match('/try again in\s+([0-9]+(?:\.[0-9]+)?)s/i', $raw, $m)) {
    ...
    private function apply_faq_schema($post_id, $faq) {
        if ($faq) {
            update_post_meta($post_id, '_amac_faq', $faq);
        }
    }
    ...
    private function generate_image_query_ai($topic, $title) {
        $prompt = "Return JSON only. Create English stock photo search query for this Ukrainian topic/title. Use concrete visible objects. Avoid abstract words. Format: {\"search\":\"...\",\"negative\":[\"...\"]}. Topic: {$topic}. Title: {$title}.";

        $r = $this->ai_call($prompt, 220, null, 1);

        $fallback = array(
            'search' => $this->fallback_image_query($topic . ' ' . $title),
            'negative' => $this->default_negative_keywords($topic)
        );
        ...
        return array(
            'search' => $this->clean_image_query($j['search']),
            'negative' => is_array($j['negative'] ?? null) ? $j['negative'] : $fallback['negative']
        );
    }
    ...
    private function image_plan($topic, $title, $data = array()) {
        ...
        if (!$search || $this->is_bad_image_query($search)) {
            $search = $this->fallback_image_query($topic . ' ' . $title);
        }
        ...
        return array(
            'search' => $this->clean_image_query($search),
            'negative' => array_unique(array_merge($negative, $this->default_negative_keywords($topic)))
        );
    }
    ...
    private function fallback_image_query($source) {
        ...
        $map = array(
            'жіноча білизна'=>'women lingerie lace underwear fashion',
            'нижня білизна'=>'women lingerie lace underwear',
            'білизна'=>'lingerie lace underwear',
            'трусики'=>'women panties underwear fashion',
            'бюстгальтер'=>'bra lingerie fashion',
            'купальник'=>'women swimsuit beachwear',
            'піжама'=>'silk pajamas sleepwear',
            'корсет'=>'corset lingerie fashion',
            'панчохи'=>'women stockings fashion',
            'спорт'=>'fitness gym workout sports equipment',
            'фітнес'=>'fitness gym workout',
            'тренування'=>'fitness workout exercise',
            'йога'=>'yoga mat exercise',
            'футбол'=>'football soccer sport',
            'баскетбол'=>'basketball court sport',
            'авто'=>'car automobile vehicle',
            'автомобіль'=>'car automobile vehicle',
            'ремонт авто'=>'car repair mechanic garage',
            'шини'=>'car tires garage',
            'двигун'=>'car engine mechanic',
            'книги'=>'books reading library',
            'книга'=>'book reading library',
            'дітей'=>'children books reading education',
            'освіта'=>'education classroom learning',
            'промисловість'=>'industrial factory manufacturing',
            'завод'=>'industrial factory manufacturing',
            'виробництво'=>'manufacturing factory production',
            'обладнання'=>'industrial equipment machinery',
            'елеватор'=>'grain elevator silo agriculture',
            'агро'=>'agriculture field farming',
            'зерно'=>'grain wheat agriculture',
            'ферма'=>'farm agriculture field',
            'сімейний бюджет'=>'family budget personal finance',
            'бюджет'=>'personal finance budget',
            'фінанси'=>'personal finance money',
            'гроші'=>'money personal finance',
            'wordpress'=>'wordpress website',
            'seo'=>'seo marketing analytics',
            'сайт'=>'website design screen'
        );

        foreach ($map as $k => $v) {
            if (mb_strpos($source_l, $k) !== false) {
                return $v;
            }
        }

        if (preg_match('/[А-Яа-яІіЇїЄєҐґ]/u', $source_l)) {
            return 'relevant editorial photo';
        }
    ...
    private function clean_image_query($q) {
        ...
        $parts = array_filter(explode(' ', $q), function($w) use ($bad) {
            return $w !== '' && !in_array($w, $bad, true);
        });
    ...
    private function default_negative_keywords($topic = '') {
        ...
        if (mb_strpos($t, 'комп') !== false || mb_strpos($t, 'ноут') !== false || mb_strpos($t, 'офіс') !== false) {
            return array('cow','landscape','random','abstract');
        }
    ...
    private function photo_is_allowed($text, $negative) {
        ...
        foreach ((array)$negative as $bad) {
            ...
            if ($bad && preg_match('/\b'.preg_quote($bad,'/').'\b/i', $text)) {
                return false;
            }
        }
    ...
    private function image_pexels($q, $exclude = array(), $negative = array()) {
        ...
        foreach (($d['photos'] ?? array()) as $p) {
            ...
            if ($u && !in_array($u, $exclude, true) && $this->photo_is_allowed($txt, $negative)) {
    ...
    private function image_pixabay($q, $exclude = array(), $negative = array()) {
        ...
        $r = wp_remote_get(add_query_arg(array(
            ...
            'per_page' => 15,
            'lang' => 'en'
        ), 'https://pixabay.com/api/'), array('timeout' => 15));
    ...
    private function image_unsplash($q, $exclude = array(), $negative = array()) {
        $k = get_option('amac_unsplash_key', '');
        ...
        $r = wp_remote_get(add_query_arg(array(
            ...
            'content_filter' => 'high',
            'client_id' => $k
        ), 'https://api.unsplash.com/photos/random'), array('timeout' => 15));
    ...
    private function image_openverse($q, $exclude = array(), $negative = array()) {
        $r = wp_remote_get(add_query_arg(array(
            ...
            'extension' => 'jpg'
        ), 'https://api.openverse.engineering/v1/images/'), array('timeout' => 15));
    ...
    private function add_inline_images($post_id, $plan, $title) {
        ...
        $used = array();
        $positions = array_unique(array(
            (int)floor(count($parts) * .35),
            (int)floor(count($parts) * .65)
        ));
    ...
    private function add_comments($post_id, $title) {
        $r = $this->ai_call("Generate 4 short realistic Ukrainian comments for article '{$title}'. One per line. No numbering.", 300, null, 1);
        ...
        foreach (array_slice(array_filter(array_map('trim', explode("\n", $r))), 0, 4) as $c) {
            ...
            wp_insert_comment(array(
                ...
                'comment_content' => wp_kses_post($c),
                'comment_approved' => 1,
                'comment_author' => $names[array_rand($names)],
                'comment_author_email' => 'user'.rand(100,999).'@example.com'
            ));
        }
    }
    ...
    private function link_cluster($ids) {
        foreach ($ids as $id) {
            ...
            if ($links) {
                wp_update_post(array(
                    'ID' => $id,
                    'post_content' => $content.'<h2>Читайте також</h2><ul>'.$links.'</ul>'
                ));
            }
        }
    }
    ...
    public function github_plugin_info($false, $action, $args) {
        if ($action !== 'plugin_information' || $args->slug !== 'ai-mass-article-creator') {
            return $false;
        }
        ...
        return (object) array(
            ...
            'tested' => '6.5',
            'sections' => array(
                'description' => 'AI SEO article generator with thematic images, SEO, FAQ Schema and internal linking.',
                'changelog' => $r->body
            ),
        );
    }
    ...
    private function github_info() {
        ...
        if (isset($d['tag_name'], $d['zipball_url'])) {
            return (object) array(
