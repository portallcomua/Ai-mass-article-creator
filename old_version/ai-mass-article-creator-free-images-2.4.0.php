...
class AI_Mass_Article_Creator {
    ...
    public function handle_settings_save() {
        ...
        if (isset($_POST['pixabay_key'])) {
            update_option('amac_pixabay_key', sanitize_text_field($_POST['pixabay_key']));
        }
            ...
            <div class="amac-grid">
                <div class="amac-card">
                    ...
                    <form method="post" action="<?php echo esc_url(admin_url('admin.php?page=amac-settings')); ?>">
                        ...
                        <hr>
                        <h3>🖼️ Безкоштовні фото для статей</h3>
                        <p style="margin-top:0;">Рекомендовано заповнити хоча б <strong>Pexels API Key</strong>. Усі ці сервіси мають безкоштовний доступ. Плагін шукає фото по черзі: <strong>Pexels → Pixabay → Unsplash</strong>. Якщо тематичне фото не знайдено — випадкові картинки не вставляються.</p>
                        <div class="amac-form-group">
                            <label>Pexels API Key <small>(рекомендовано, безкоштовно)</small>:</label>
                            <input type="password" name="pexels_key" value="<?php echo esc_attr($pexels_key); ?>">
                            <small>Де взяти: <a href="https://www.pexels.com/api/" target="_blank">pexels.com/api</a> → Join / Get started → скопіюйте API Key. Безкоштовний ліміт зазвичай достатній для масової генерації невеликими партіями.</small>
                        </div>
                        <div class="amac-form-group">
                            <label>Pixabay API Key <small>(додатковий безкоштовний запас)</small>:</label>
                            <input type="password" name="pixabay_key" value="<?php echo esc_attr($pixabay_key); ?>">
                            <small>Де взяти: <a href="https://pixabay.com/api/docs/" target="_blank">pixabay.com/api/docs</a> → увійти в акаунт → ваш key показаний у документації.</small>
                        </div>
                        <div class="amac-form-group">
                            <label>Unsplash Access Key <small>(резервний варіант)</small>:</label>
                            <input type="password" name="unsplash_key" value="<?php echo esc_attr($unsplash_key); ?>">
                            <small>Де взяти: <a href="https://unsplash.com/developers" target="_blank">unsplash.com/developers</a> → Register as Developer → New Application → Access Key.</small>
                        </div>
                        <div class="amac-form-group">
    ...
    private function get_unsplash_image_url($keyword, $width = 1200, $height = 800) {
        // Backward compatible name, але тепер це універсальний пошук тематичного фото.
        return $this->get_thematic_image_url($keyword, $width, $height);
    }

    private function get_thematic_image_url($keyword, $width = 1200, $height = 800) {
        ...
        // 1) Pexels — найкращий перший варіант для статей.
        $url = $this->search_pexels_image($keyword, $width, $height);
        if ($url) return $url;

        // 2) Pixabay — хороший резерв, особливо для товарних/нішевих тем.
        $url = $this->search_pixabay_image($keyword, $width, $height);
        if ($url) return $url;

        // 3) Unsplash — резервний варіант.
        $url = $this->search_unsplash_image($keyword, $width, $height);
        if ($url) return $url;

        // ВАЖЛИВО: не повертаємо random/picsum/computer/business.
        // Якщо тематичного фото немає — не додаємо фото, щоб не псувати статтю.
        return false;
    }

    private function search_pexels_image($keyword, $width = 1200, $height = 800) {
        $pexels_key = get_option('amac_pexels_key', '');
        if (empty($pexels_key)) return false;
        ...
        if (empty($data['photos']) || !is_array($data['photos'])) return false;
        ...
        foreach ($data['photos'] as $photo) {
            ...
            if (!empty($photo['src']['original'])) return esc_url_raw($photo['src']['original']);
        }

        return false;
    }

    private function search_pixabay_image($keyword, $width = 1200, $height = 800) {
        $pixabay_key = get_option('amac_pixabay_key', '');
        if (empty($pixabay_key)) return false;
        ...
        $data = json_decode(wp_remote_retrieve_body($response), true);
        if (empty($data['hits']) || !is_array($data['hits'])) return false;
    ...
    private function search_unsplash_image($keyword, $width = 1200, $height = 800) {
        $unsplash_key = get_option('amac_unsplash_key', '');
        if (empty($unsplash_key)) return false;
        ...
        if (!empty($data['urls']['full'])) return esc_url_raw($data['urls']['full']);
